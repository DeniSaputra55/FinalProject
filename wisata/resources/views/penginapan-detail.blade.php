...
<div class="site-section">
    <div class="container">
        <div class="row">
            <div class="col-md-6">
            <div class="penginapan-details">
  <h2 class="listing-title text-center">{{ $penginapan->nama }}</h2>
  <table class="table">
    <tbody>
      <tr>
        <td><i class="fas fa-star"></i> Kelas</td>
        <td>{{ $penginapan->kelas }}</td>
      </tr>
      <tr>
      ...
      <tr>
        <td><i class="fas fa-info-circle"></i> Fasilitas</td>
        <td>{{ $penginapan->fasilitas }}</td>
      </tr>
      <tr>
        <td><i class="material-icons locationIcon" style="color: #000000;">place</i> Alamat</td>
        <td>{{ $penginapan->alamat }}</td>
            ...
            <div class="col-md-6">
                <img src="{{ asset('admin/image/'.$penginapan->foto) }}" alt="{{$penginapan->foto}}" class="img-thumbnail" style="box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
            </div>
        </div>
    </div>
</div>
...
<script>
    const cekCuaca = async() => {
        const url = 'http://api.weatherapi.com/v1/current.json?key=ca34b190634a4ca1a0c140031232106&q={{ $penginapan->kota }}&aqi=no';
